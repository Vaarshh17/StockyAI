...
class TestRunAgent:
    @pytest.mark.asyncio
    async def test_simple_response_no_tools(self, reset_memory, reset_persona):
        with patch("agent.core.call_llm", new=AsyncMock(return_value={
    ...
    @pytest.mark.asyncio
    async def test_tool_call_then_final_response(self, reset_memory, reset_persona):
        tool_call_response = {
            "content": None,
            "tool_calls": [{
                "id": "call_1",
    ...
    @pytest.mark.asyncio
    async def test_draft_message_detection(self, reset_memory, reset_persona):
        with patch("agent.core.call_llm", new=AsyncMock(return_value={
    ...
    @pytest.mark.asyncio
    async def test_max_tool_iterations(self, reset_memory, reset_persona):
        always_tool_call = {
            "content": None,
            "tool_calls": [{
                "id": "call_x",
    ...
    @pytest.mark.asyncio
    async def test_forwarded_message_formatting(self, reset_memory, reset_persona):
        with patch("agent.core.call_llm", new=AsyncMock(return_value={
            "content": "Noted.",
            "tool_calls": None,
        })):
    ...
    @pytest.mark.asyncio
    async def test_history_saved_after_response(self, reset_memory, reset_persona):
        with patch("agent.core.call_llm", new=AsyncMock(return_value={
            "content": "Hello!",
            "tool_calls": None,
        })):
...
class TestRunProactiveBrief:
    @pytest.mark.asyncio
    async def test_morning_brief(self, reset_memory, reset_persona):
        with patch("agent.core.call_llm", new=AsyncMock(return_value={
    ...
    @pytest.mark.asyncio
    async def test_spoilage_uses_fast_model(self, reset_memory, reset_persona):
        mock_llm = AsyncMock(return_value={
            "content": "No spoilage risk.",
            "tool_calls": None,
        })
        with patch("agent.core.call_llm", new=mock_llm):
            with patch("agent.core.get_persona", return_value=None):
                with patch("agent.core.load_persona", new=AsyncMock(return_value=None)):
                    from agent.core import run_proactive_brief
    ...
    @pytest.mark.asyncio
    async def test_digest_appends_instinct(self, reset_memory, reset_persona):
        with patch("agent.core.call_llm", new=AsyncMock(return_value={
